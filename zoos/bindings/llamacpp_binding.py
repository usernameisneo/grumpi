...
class LlamaCppBinding(Binding):
    ...
    def get_binding_config(cls) -> Dict[str, Any]:
        ...
        return {
            "type_name": cls.binding_type_name, "version": "1.2",
            "description": "Binding for llama.cpp GGUF models via its HTTP server.", "supports_streaming": True,
            "requirements": ["llama-cpp-python>=0.2.60", "requests"],
            "config_template": {
                "type": {"type": "string", "value": cls.binding_type_name, "required":True},
                "models_folder": {"type": "string", "value": "models/llama_cpp_models/", "description": "Folder with GGUF models.", "required": True},
                "server_path": {"type": "string", "value": None, "description": "Optional path to 'server' executable.", "required": False},
                "n_gpu_layers": {"type": "int", "value": -1, "required": False}, "n_ctx": {"type": "int", "value": 2048, "required": False},
                "batch_size": {"type": "int", "value": 512, "required": False}, "threads": {"type": "int", "value": 0, "required": False},
                "threads_batch": {"type": "int", "value": 0, "required": False}, "tensor_split": {"type": "string", "value": None, "required": False},
                "cache_type": {"type": "string", "value": "fp16", "options":["fp16", "q8_0", "q4_0"], "required": False},
                "compress_pos_emb": {"type":"float", "value":1.0, "required":False}, "rope_freq_base": {"type":"float", "value":0.0, "required":False},
                "additional_args": {"type":"dict", "value": {}, "description":"Extra flags (e.g. {'--mlock':True})", "required":False}
            }
        }
    ...
    async def list_available_models(self) -> List[Dict[str, Any]]:
        ...
        if not self.models_folder.is_dir(): logger.error(f"Models folder not found: {self.models_folder}"); return []
        available_models = []
        for gguf_file in self.models_folder.glob("*.gguf"):
            if gguf_file.is_file():
                try:
                    stat_info = gguf_file.stat(); model_name = gguf_file.name; is_vision = "llava" in model_name.lower()
                    model_data = { "name": model_name, "size": stat_info.st_size, "modified_at": datetime.fromtimestamp(stat_info.st_mtime), "supports_vision": is_vision, "supports_audio": False, "format": "gguf", "details": {"full_path": str(gguf_file.resolve())} }
                    available_models.append(model_data)
                except Exception as e: logger.warning(f"Could not read metadata for {gguf_file}: {e}")
    ...
    async def load_model(self, model_name: str) -> bool:
        ...
        async with self._load_lock:
            ...
            if not target_model_path.is_file(): logger.error(f"{self.binding_name}: Model file not found: {target_model_path}"); return False
            if self._model_loaded and self.current_model_full_path != target_model_path:
                 logger.info(f"{self.binding_name}: Different model request. Stop server for {self.model_name}..."); await self.unload_model()
            elif self._model_loaded and self.server_process and self.server_process.poll() is None:
                 logger.info(f"{self.binding_name}: Model '{model_name}' server already running."); return True

            logger.info(f"{self.binding_name}: Starting server for model: {model_name}")
            self.server_port = self._find_available_port(); logger.info(f"{self.binding_name}: Using port {self.server_port}")
            ...
            cmd = [ server_executable, "--model", str(target_model_path), "--ctx-size", str(self.n_ctx), "--n-gpu-layers", str(self.n_gpu_layers), "--batch-size", str(self.batch_size), "--port", str(self.server_port), ]
            ...
            try:
                async with resource_context:
                    ...
                    self.model_name = model_name; self.current_model_full_path = target_model_path; self._model_loaded = True
                    logger.info(f"{self.binding_name}: Server started for {model_name}.")
                    return True
            except asyncio.TimeoutError: logger.error(f"{self.binding_name}: Timeout wait GPU."); await self._terminate_server(); return False
    ...
    def _monitor_stderr(self, stream):
        ...
        except Exception as e: logger.debug(f"Stderr monitor stop {self.binding_name}: {e}")
        finally:
    ...
    async def _terminate_server(self):
        ...
        if self.server_process and self.server_process.poll() is None:
            logger.info(f"{self.binding_name}: Terminate server {self.server_process.pid}...")
            try: self.server_process.terminate(); await asyncio.wait_for(self.server_process.wait(), timeout=5); logger.info(f"{self.binding_name}: Server process terminated.")
            ...
            except Exception as e: logger.error(f"{self.binding_name}: Error terminate server: {e}")
    ...
    async def unload_model(self) -> bool:
        ...
        async with self._load_lock:
            ...
            self.model_name = None; self._model_loaded = False; self.current_model_full_path = None
    ...
    async def _api_call(self, endpoint: str, payload: Dict, timeout: int = 60) -> Dict:
        """Internal helper to make sync API calls async."""
        if not self.server_port: raise RuntimeError("Server not running")
        url = f"http://{self.server_host}:{self.server_port}/{endpoint}"
        try:
            response = await asyncio.to_thread(self.session.post, url, json=payload, timeout=timeout); response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e: logger.error(f"{self.binding_name}: API Error ({endpoint}): {e}"); raise RuntimeError(f"Server request error: {e}") from e
        except Exception as e: logger.error(f"{self.binding_name}: Error ({endpoint}): {e}", exc_info=True); raise RuntimeError(f"Unexpected API error: {e}") from e
    ...
    def _prepare_llamacpp_payload(self, prompt: str, params: Dict[str, Any], stream: bool, multimodal_data: Optional[List['InputData']] = None) -> Optional[Dict[str, Any]]:
        ...
        if multimodal_data and self.get_supported_input_modalities() == ["text", "image"]:
            ...
            if image_parts:
                ...
                payload["image_data"] = [{"data": img.data, "id": idx + 10} for idx, img in enumerate(image_parts) if img.data]
                if not any(f"[img-{i+10}]" in payload["prompt"] for i in range(len(image_parts))): logger.warning("LLaVA prompt missing image placeholders.")
        return payload
    ...
    async def generate( self, prompt: str, params: Dict[str, Any], request_info: Dict[str, Any], multimodal_data: Optional[List['InputData']] = None ) -> Union[str, Dict[str, Any]]:
        ...
        if params.get("auto_max_new_tokens", False) and payload.get("n_predict", -1) < 0:
            est_prompt_tokens = len(payload["prompt"].split()); max_new = self.n_ctx - est_prompt_tokens - 4; payload["n_predict"] = max(1, max_new); logger.info(f"Auto-setting n_predict to {payload['n_predict']}")
        result = await self._api_call("completion", payload, timeout=DEFAULT_LLAMACPP_TIMEOUT); completion = result.get("content", "")
        logger.info(f"{self.binding_name}: Generation successful."); return {"text": completion.strip()}
    ...
    async def generate_stream( self, prompt: str, params: Dict[str, Any], request_info: Dict[str, Any], multimodal_data: Optional[List['InputData']] = None ) -> AsyncGenerator[Dict[str, Any], None]:
         ...
         if params.get("auto_max_new_tokens", False) and payload.get("n_predict", -1) < 0:
             est_prompt_tokens = len(payload["prompt"].split()); max_new = self.n_ctx - est_prompt_tokens - 4; payload["n_predict"] = max(1, max_new); logger.info(f"Auto-setting n_predict to {payload['n_predict']}")
         url = f"http://{self.server_host}:{self.server_port}/completion"; full_response_text = ""
         try:
             async with httpx.AsyncClient(timeout=DEFAULT_LLAMACPP_TIMEOUT) as client:
                  async with client.stream("POST", url, json=payload) as response:
                       response.raise_for_status()
                       async for line in response.aiter_lines():
                            if not line or not line.startswith('data: '): continue; line = line[6:]
                            try:
                                 ...
                                 if data.get('stop', False): logger.info(f"{self.binding_name}: Stream finished."); metadata = {"timings": data.get("timings")} if "timings" in data else {}; yield {"type": "final", "content": {"text": full_response_text}, "metadata": metadata}; break
    ...
    async def tokenize(self, text: str, add_bos: bool = True, add_eos: bool = False) -> List[int]:
        ...
        if not self.server_instance: raise RuntimeError("Server not running for tokenization")
        payload = {"content": text, "add_bos": add_bos, "add_eos": add_eos}
        result = await self._api_call("tokenize", payload)
        tokens = result.get("tokens")
        if tokens is None or not isinstance(tokens, list): raise RuntimeError("Tokenization failed or returned invalid format.")
        return tokens

    async def detokenize(self, tokens: List[int]) -> str:
        """Detokenizes tokens using the llama.cpp server's /detokenize endpoint."""
        if not self.server_instance: raise RuntimeError("Server not running for detokenization")
        payload = {"tokens": tokens}
        result = await self._api_call("detokenize", payload)
        text = result.get("content")
        if text is None: raise RuntimeError("Detokenization failed or returned invalid format.")
        return text

    async def get_current_model_info(self) -> Dict[str, Any]:
        """Returns information about the currently loaded model."""
        if not self._model_loaded or not self.model_name: return {}
        is_vision = "llava" in self.model_name.lower()
        return {
            "name": self.model_name,
            "context_size": self.n_ctx,
            "max_output_tokens": -1, # llama.cpp server doesn't expose this easily
            "supports_vision": is_vision,
            "supports_audio": False,
            "details": {"full_path": str(self.current_model_full_path)}
        }
    # --- END NEW METHODS ---
    ...
    def __del__(self):
         if self.server_process and self.server_process.poll() is None: logger.warning(f"LlamaCppBinding del without stop. Terminate {self.server_process.pid}.")
